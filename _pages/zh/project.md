...
* **多源状态估计**：融合 IMU、轮速编码器与关节编码器数据；实现 2 维卡尔曼滤波估计前进速度，在腾空等轮速不可靠阶段采用预测值替代观测。

* **弹簧力补偿**：对并联拉簧建立三次多项式力-位移模型，并将其映射为关节补偿力矩，提升支撑力估计精度。

* **分层状态机**：实现基于优先级仲裁的状态机，覆盖死亡、恢复、正常、飞行、跳跃、上单级台阶、上多级台阶、爬行模式等模式，具备自动恢复与安全倒地检测。
...
* **CAN 监控接口**：通过 FDCAN 与机器人底盘建立双向通信，接收电池功率指令并回传电容电压、电流及剩余电量百分比等状态信息。
